...
- **THEN** adapter 与 baseUrl 保持用户改写后的取值，不被缺省启发或目录预填覆盖；且改写 adapter/baseUrl 时已选 provider/model 及其展示信息不被清空或改动，服务商 combobox SHALL NOT 被重新聚焦或重新展开（未手改 baseUrl 前，改选其他 model 仍按目录预填）

#### Scenario: 添加弹窗不自动聚焦服务商搜索
- **WHEN** 用户打开工作区 provider 添加弹窗
- **THEN** 服务商搜索框不被程序化聚焦，选项列表保持收起，直到用户主动聚焦或输入
