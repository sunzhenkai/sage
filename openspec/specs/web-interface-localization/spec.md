...
`web-interface-localization` 资源 SHALL为工作区 provider 保存/删除/失败提示、provider 必需引导、provider 添加/编辑弹窗（含 Catalog 辅助选择、不可用降级与同 provider 重复添加）以及工作区默认模型相关文案提供 message key，且 `zh-CN` 与 `en` 的 key 集保持一致。资源 SHALL NOT 再保留存量外部配置（browser-local profile）弃用提示相关键。所有 Web 代码中呈现该类提示时 SHALL通过 `t(key, values)` 调用，不得直接拼接英文句子。
