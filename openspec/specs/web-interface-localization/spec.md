# web-interface-localization Specification

## Purpose
TBD - created by archiving change web-interface-localization. Update Purpose after archive.
## Requirements
...
`web-interface-localization` 资源 SHALL为工作区 provider 保存/删除/失败提示与 provider 必需引导（含存量外部配置弃用提示）提供 message key，且 `zh-CN` 与 `en` 的 key 集保持一致。所有 Web 代码中呈现该类提示时 SHALL通过 `t(key, values)` 调用，不得直接拼接英文句子。
...
- **WHEN** Chat 无可用工作区 provider 或 Providers 页检测到存量浏览器本地外部配置
- **THEN** 使用 `t('chatNeedsProvider')` 与 `t('legacyProfilesNotice')` 等既有键，zh/en 语义一致
