...
代码库 SHALL 在 `platform/examples/ai-apps/` 下提供一个专用于生命周期验证的测试 AI App 源包，其目录即包结构 SHALL 符合 `AgentSourceManifest.v1` 清单契约（合法 `id`/`version`/`entry`，`entry` 指向包内 `prompts/*.md`），且其运行输出 SHALL 是确定性的、可精确断言的，不得依赖外部网络、真实模型或当前时间等不稳定因素。
...
- **WHEN** 在受信测试开关启用 fake live provider 的环境下多次运行该测试 App
