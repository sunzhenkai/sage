...
describe('run output artifact resolver', () => {
  it('returns the reference with content when the materialized output matches', async () => {
    const resolver = createRunOutputArtifactResolver({
      ...
      lookup: { async getRunOutput(_tenant, taskId) { expect(taskId).toBe('task-1'); return { artifactRef: reference.artifactRef, output: '{"overview":"digest"}', mediaType: 'application/json' }; } }
    });
  ...
  it('returns the bare reference when the stored artifactRef differs and never throws on lookup failure', async () => {
    const stale = createRunOutputArtifactResolver({
      tenantId: 'tenant-local',
      lookup: { async getRunOutput() { return { artifactRef: 'artifact://tasks/task-1/attempt-2/slice-1' as const, output: 'stale', mediaType: 'text/plain' }; } }
    });
  ...
  it('resolves declared file-name references to the same run output content', async () => {
    const output = { tenantId: 'tenant-local', taskId: 't1', artifactRef: 'artifact://tasks/t1/attempt-1/slice-1' as const, output: 'body', mediaType: 'text/plain', createdAt: '2026-08-29T00:00:00.000Z' };
