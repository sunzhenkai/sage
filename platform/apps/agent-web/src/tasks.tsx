...
const STATUS_FILTERS: readonly (readonly [string, 'allStatuses' | 'statusRunning' | 'statusPaused' | 'statusFailed' | 'statusSucceeded' | 'statusCancelled'])[] = [['all', 'allStatuses'], ['running', 'statusRunning'], ['paused', 'statusPaused'], ['failed', 'statusFailed'], ['succeeded', 'statusSucceeded'], ['cancelled', 'statusCancelled']];
...
export function ProjectionFreshness({ task }: { readonly task: TaskViewModel }) { const { locale, t, formatDateTime, formatCompact } = useLocale(); return <div className={`freshness freshness-${task.freshness}`} data-testid="projection-freshness"><span className="freshness-dot" /><span className="freshness-label">{t('projection')}</span><strong>{locale === 'en' ? task.freshness : statusLabel(task.freshness, t)}</strong>{task.projectionUpdatedAt && <time dateTime={task.projectionUpdatedAt} title={formatDateTime(task.projectionUpdatedAt)}>{formatCompact(task.projectionUpdatedAt)}</time>}{task.staleReason && <small>{task.staleReason === 'age_threshold_exceeded' ? t('staleAgeThreshold') : task.staleReason.replaceAll('_', ' ')}</small>}</div>; }
...
export function TasksApp({ apiBase = '', fetcher = fetch, sessionId, taskId }: { readonly apiBase?: string; readonly fetcher?: TaskFetch; readonly sessionId?: string; readonly taskId?: string }) {
  ...
  return <section className="workspace-page tasks-page" aria-label={t('tasks')}><header className="page-heading"><h1>{t('tasks')}</h1><div className="page-heading-actions"><span className="metric-chip"><strong>{tasks.filter((task) => task.status === 'running').length}</strong> {t('runningCount', { count: tasks.filter((task) => task.status === 'running').length }).replace(/^\d+\s*/, '')}</span>{selected === undefined ? <button className="button button-secondary" type="button" onClick={() => void load()}>↻ {t('refresh')}</button> : null}</div></header>{error ? <Banner kind="error" title={t('taskDataUnavailable')}>{error}</Banner> : selected ? <>{detailLoading && <div className="loading-strip"><span className="loading-spinner" />{t('refreshingTask')}</div>}<TaskDetail task={selected} events={events} artifacts={artifacts} {...(runLogs === undefined ? {} : { runLogs })} runLogsLoadingMore={loadingMoreRunLogs} runLogsError={runLogsError} detailLoading={detailLoading} {...(sessionId ? { sessionId } : {})} onControl={(operation) => void control(operation)} onRefresh={() => void select(selected.taskId)} onSelectAttempt={(runId, attemptId) => void runLogPage('attempt', runId, attemptId)} onLoadMoreRunLogs={() => void runLogPage('more')} /></> : <><div className="task-toolbar"><label className="search-field"><span>⌕</span><input aria-label={t('searchTasks')} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('searchTaskPlaceholder')} /></label><div className="chip-group" role="group" aria-label={t('status')}>{STATUS_FILTERS.map(([value, labelKey]) => <button className={statusFilter === value ? 'chip is-active' : 'chip'} key={value} type="button" aria-pressed={statusFilter === value} onClick={() => setStatusFilter(value)}>{t(labelKey)}</button>)}</div></div>{loading ? <LoadingState label={t('loading')} detail={t('readingDurableState')} /> : <TaskList tasks={visibleTasks} {...(sessionId ? { sessionId } : {})} />}</>}</section>;
}
