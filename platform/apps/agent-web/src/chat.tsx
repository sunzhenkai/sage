...
export function ChatTimeline({ events, onRetry, onPromote }: ChatTimelineProps) {
  const { t } = useLocale();
  if (events.length === 0) return <section className="timeline-empty" aria-live="polite"><p className="empty-title">{t('chatEmptyTitle')}</p></section>;
...
export function ChatApp({ sessionId, apiBase = '', fetcher = fetch }: ChatAppProps) {
  ...
  const submitGuard = useRef(false); const composition = useRef(false); const hasTask = useMemo(() => events.some((event) => event.payload.kind === 'task' && event.payload.taskId !== undefined), [events]); const sessionWritable = sessionStatus === 'open' && !archived;
  ...
  return <section className="workspace-page content-pane chat-page">
    <header className="page-heading chat-heading">
      <div className="chat-heading-row">
        ...
        <div className="chat-heading-tools"><label className="runtime-picker"><span className="overview-label">{t('runtime')}</span><select aria-label={t('chatRuntime')} value={runtimeId} onChange={(event) => selectRuntime(event.target.value)}><option value="">{t('runtimeUnconfigured')}</option>{workspaceConnections.length > 0 && <optgroup label={t('workspaceProviders')}>{workspaceConnections.map((connection) => <option key={`${WS_RUNTIME_PREFIX}${connection.id}`} value={`${WS_RUNTIME_PREFIX}${connection.id}`}>{connection.name}{connection.modelName === undefined ? '' : ` · ${connection.modelName}`}</option>)}</optgroup>}</select></label>{workspaceConnections.length === 0 && <a className="task-workspace-link" href={workspaceHref({ view: 'providers', sessionId })}>{t('addWorkspaceProvider')}</a>}{events.length > 0 && !hasTask && <a className="button button-secondary task-card-link" href={workspaceHref({ view: 'tasks', sessionId })} title={t('taskCardBody')}><span aria-hidden="true">▣</span>{t('promoteTask')}</a>}<a className="task-workspace-link" href={workspaceHref({ view: 'tasks', sessionId })}>{t('openTaskWorkspace')} <span aria-hidden="true">→</span></a><button className="button button-secondary stream-toggle" type="button" aria-expanded={streamOpen} aria-controls="chat-event-stream" onClick={() => setStreamOpen((open) => !open)}>{t('eventStream')}</button></div>
      </div>
    </header>
