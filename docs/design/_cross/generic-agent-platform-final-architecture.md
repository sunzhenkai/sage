...
- **日期**：2026-08-15
- **状态**：Proposed final architecture baseline
- **范围**：Sage 通用 Agent 平台目标态逻辑架构
- **性质**：决策级目标方案，不代表当前实现已经具备全部能力
- **当前实现基线**：[`first-version-system-architecture.md`](../first-version-system-architecture.md)
- **现有开放问题**：[`open-questions.md`](../open-questions.md)
...
| 方案 | 描述 | 优点 | 主要问题 | 结论 |
|------|------|------|----------|------|
| A. Package/DAG-centric | Package 编译为固定 DAG，模型作为节点 | 实现快、流程直观 | 认知逻辑碎片化，新 workload 需要新 DAG；难以保持 Chat/Task 语义一致 | 拒绝作为核心；保留为确定性子流程 |
| **B. Agent-first Kernel + Engine Adapters** | Kernel 统一平台语义，Engine 承载认知机制，Package 编译为不可变执行 Spec | Agent 自主性、框架可替换、可增量复用 Sage 现有能力 | 需要严格 Broker 和 conformance contract | **采用** |
| C. Remote Agent Runtime per framework | 每种框架独立远程 Runtime，平台只提供 Gateway | 多语言和强隔离最好 | 网络面、双重 retry、语义漂移和分布式事务复杂 | 作为未来特殊部署模式，不作为主架构 |
| D. Temporal-centric Agent Workflow | 把 Agent step 固定成 Temporal Workflow/Activity 结构 | 与当前实现最接近 | Coordinator 语义泄漏到核心，难支持 Interactive/其他 runtime | 仅作为 Durable Adapter 实现 |

**推荐方案：B。**

接受的代价：需要新增统一 Spec、Broker、Receipt、sealed Checkpoint 和 conformance suite；短期逻辑组件数量增加。
...
- 它将取代 [`first-version-system-architecture.md`](../first-version-system-architecture.md) 作为长期目标架构；
