...
- Schedule 触发(P8):occurrence 三层幂等(workflow ID = `schedule:{id}:occ:{n}`、admission 幂等键、task store 唯一约束)保证零静默重复;missed/skipped 由对账 activity 推算差集补记 MISSED;FOLLOW 绑定不兼容或预算超限均稳定失败并告警(fail closed,不静默跳过);
- 自动重试护栏(P8):`assertUnattendedRetryBudget` 在 task + schedule 双级预算内放行,超限 fail closed;
- EFFECT_UNKNOWN(P8):不复活终态;由 `/v1/effects/resolutions` 人工裁决(retry 新 attempt / Ledger replay / terminate),append-only 审计;
- Checkpoint:长 Run 由 agent-state-postgres 周期写 Checkpoint,可重放;
- Worker 崩溃:Worker 重启从 Temporal 拉取历史,Agent Run 从最近 Checkpoint 续跑;dispatcher 崩溃由 Temporal Schedule 下次触发 + 对账收敛;
- 多区域:暂不跨 Region;目标态见终版架构。

## 可观测

- 日志:JSON 行,worker/api 输出到 stdout(由 docker compose 收集);
- 指标:`observability` 包暴露 Prometheus 指标,Worker 内部 Counter/Histogram;P8 新增 `sage_schedule_trigger_total{outcome,reason_code}`(低基数,schedule/occurrence 标识只进日志字段);
- 追踪:Temporal 自带 Workflow/Activity Trace ID,关联 Agent Run `runtimeCorrelation`;
- 告警:P8 起自带规则生成——`production-governance/failure-taxonomy.ts` 单一映射表生成 `platform/observability/prometheus/sage-p8-alerts.yaml`(每条规则强制 responder_service 与 runbook_url 注解,未知错误码兜底告警);部署环境加载后路由到 oncall;
- SLO:暂未对外公布,验收标准见 `evidence/agent-platform-final/`;P8 运行门见 [pilot-gate 概念](../concepts/pilot-gate.md)。
